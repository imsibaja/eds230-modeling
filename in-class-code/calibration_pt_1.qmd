...
msage = readRDS(here("in-class-code/Data/msage.RDS"))
#View(msage)

# first rearrange so we can plot all results
msagel = msage %>% gather(key="run",value="str", -date, -month, -day, -year, -wy,-obs)
source(here("in-class-code/R/compute_highfhighmetrics.R"))

# another example using our high fhigh statistics
# use apply to compute for all the data
res = msage %>% select(-date, -month, -day, -year, -wy, -obs ) %>% apply(2,FUN=compute_highfhighmetrics, o=msage$obs, month=msage$month, day=msage$day, year=msage$year, wy=msage$wy)
...
# pick parameter set with greater high fhigh month correlation
...
# or with observed but focus on August fhigh
msagel_mth = msagel %>% select(-date,-day) %>%  group_by(month, wy, run, accept) %>% summarise(str=mean(str, na.rm=T), obs=mean(obs, na.rm=T))

# just august fhigh
ggplot(subset(msagel_mth,  month==2 & accept),
       aes(wy, str, group=wy))+geom_boxplot(position="dodge")+
  geom_point(aes(wy, obs), col="red")+labs(y="Streamfhigh", x="Date")

mean_august_obs = mean(subset(msagel_mth, month==2)$obs) 
ggplot(subset(msagel_mth, month==2), aes(accept, str))+geom_boxplot()+
  geom_hline(yintercept=mean_august_obs, col="red")+labs(y="Streamfhigh", x="Date")
```
